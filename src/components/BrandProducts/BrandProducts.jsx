...
export default function BrandProducts({ title }) {
  ...
  let param = useParams();
  let { isLoading, data } = useQuery("brandProducts", getProducts);
  // filter data to get the products of the brand
  function getBrandProducts() {
    const filteredProducts = data?.data.data.filter(
      (product) => product.brand.name === param.name
    );
